...
小蟹同志从波士顿回来，礼物一个桌子摆不下。

昨天跑 skill omni 自演化，用了 sonnet，烧了 600 美刀。被光阳教育了。不过我 prompt caching 省下来的钱绝对比 600 美刀多多了，应该不会扣我工资吧 😱。

前天说想吃的一家冰激淋卷，今天下班就出现在了我面前 😁

__*Week ?...*__

永久下班了。最大的感触可能是现在 agent 太牛逼了，我只要有一个 idea 就能快速验证，对于有一定经验的程序员简直如虎添翼。但是也有很多人往公司代码仓里交一个 PR 几万行的 slop。

以及几周前还觉得自演化的概念很革命性，现在看来也不尽然。把 skill 和 experience 丢给 llm 让他自己 get better 和许愿让它自己变好一样。在 MD 文档里再怎么修修改改，基模一更新又成了无用功。而且很多 Skill 自演进的方法甚至还依赖真实数据，真实场景中哪来这么多 case 给你进化。即使有了真实数据，有用的 skill 编辑充其量也只是 clarify 模糊的 problem statement。Skill 就是 skill，一个流水线而已，怎么也雕不出花儿来。顿时感觉从 engineering 换方向到 skill evolution research 的决策还有待商榷是否英明。

不过也算干了些对公司有一丢丢用的工作，不虚此行。下学期 all in ICPC，算竞老登最后一舞！
