...
&emsp;&emsp;$\max(a_x + a_y, b_y + b_x)$ 的贡献形式不是独立的，并不好进行分析。考虑通过分类讨论将 $\max$ 拆开。若令 $h_i = a_i - b_i$，$h'_i = b_i - a_i$，可以发现若 $h_x \geqslant h'_y$ 取值则为 $b_x + b_y$，反之亦然。

&emsp;&emsp;注意到 $h$ 本身自带一个一维偏序关系，于是放到数轴上用线段树维护，每个节点维护 $a_x$，$a_y$，$b_x$，$b_y$ 的最小值，以及区间的答案。修改操作在线段树的叶子节点维护四个 `std::multiset` 即可。
