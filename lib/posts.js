...
export async function getPostData(section, id) {
  ...
  const codeBlock = String(rinima.match(xCode)[0])
  // fs.writeFileSync("C:/Users/cirnovsky/Downloads/fuck.html", codeBlock)
  // console.log(codeBlock)
  rinima = rinima.replace(xCode, "{code area, there's tons of bugs in html-hast, fuck this goddamn shit.}");
  let hast = parse(rinima);

  const processedTitle = FuckYou.processSync(matterResult.data.title);
  const contentHtml = hast.map(fuckUnified).join("\n").replace(/{code area, there's tons of bugs in html-hast, fuck this goddamn shit.}/, codeBlock);
