...
export function getAllPostsData(section) {
    ...
    const fileDir = path.join(postsDirectory, (SectionFunc.getPath(section) || "caonimabi"))
...
export async function getPostData(section, id) {
    const fullPath = path.join(postsDirectory, SectionFunc.getPath(section), `${id}.md`);
    const fileContents = fs.readFileSync(fullPath, 'utf-8');

    const matterResult = matter(fileContents);
    const processedContent = FuckYou.processSync(matterResult.content)
    let hast = parse(processedContent.toString());
